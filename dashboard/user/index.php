...
<div class="bg-white shadow-md rounded-lg p-6 mb-6">
    ...
    <div class="flex flex-col md:flex-row gap-4 mb-6">
        <a href="report-lost.php" class="bg-red-600 hover:bg-red-700 text-white py-2 px-4 rounded-lg flex-1 text-center">
            <i class="fas fa-search mr-2"></i> Report Lost Item
        </a>
        <a href="report-found.php" class="bg-green-600 hover:bg-green-700 text-white py-2 px-4 rounded-lg flex-1 text-center">
    ...
    <?php if (count($user_items) > 0): ?>
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6" id="items-container">
            <?php foreach($user_items as $item): ?>
                <div class="border rounded-lg overflow-hidden flex item-card" 
                     data-type="<?php echo $item['type']; ?>" 
                     data-category="<?php echo $item['category']; ?>">
                    <div class="w-1/3 bg-gray-200">
                        <?php if ($item['image']): ?>
                            <img src="<?php echo UPLOAD_URL . $item['image']; ?>" alt="<?php echo $item['title']; ?>" class="w-full h-full object-cover">
                        <?php else: ?>
                    ...
                    <div class="w-2/3 p-4">
                        <div class="flex justify-between items-start">
                            ...
                            <?php if ($item['match_count'] > 0): ?>
                                <span class="bg-yellow-100 text-yellow-800 text-xs font-semibold px-2.5 py-0.5 rounded">
                                    <?php echo $item['match_count']; ?> potential <?php echo $item['match_count'] == 1 ? 'match' : 'matches'; ?>
                                </span>
                            <?php endif; ?>
                        </div>
                        <h3 class="text-lg font-bold mb-1"><?php echo $item['title']; ?></h3>
                        <p class="text-gray-600 mb-2 text-sm"><?php echo substr($item['description'], 0, 100) . (strlen($item['description']) > 100 ? '...' : ''); ?></p>
                        ...
                        <div class="mt-3 flex justify-between items-center">
                            <span class="text-xs text-gray-500">
                                <i class="far fa-calendar-alt mr-1"></i> <?php echo date('M d, Y', strtotime($item['date_lost_found'])); ?>
                            </span>
                            <a href="item-details.php?id=<?php echo $item['id']; ?>" class="text-blue-600 hover:text-blue-800 text-sm font-medium">
                                View Details
                            </a>
                        </div>
                    </div>
